...
They can be used with the text generation pipeline:

```py
from transformers import pipeline

pipe = pipeline(task="text-generation", model=model, tokenizer=tokenizer)
print(pipe("AI is going to"))
```

You can use the generation [parameters](https://huggingface.co/docs/transformers/main/en/main_classes/text_generation#transformers.GenerationConfig) available in 🤗 Transformers:

```py
pipe("AI is going to", max_new_tokens=256, do_sample=True, temperature=0.8, repetition_penalty=1.1)
```

You can also use a 🤗 tokenizer:
